...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    ...
    meanSpeedMetеrPerSecod := meanSpeed(action, duration) * kmhInMsec
    heightInMetеr := height/cmInM
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(meanSpeedMetеrPerSecod, 2) / heightInMetеr) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
